...
void setup() {
  ...
  server.on("/hotspot-detect.html", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", "No internet");
    Serial.println("Responded to /hotspot-detect.html");
  });

  server.on("/library/test/success.html", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", "No internet");
    Serial.println("Responded to /library/test/success.html");
  });

  server.on("/ncsi.txt", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", "No internet");
    Serial.println("Responded to /ncsi.txt");
  });

  server.on("/connecttest.txt", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", "No internet");
    Serial.println("Responded to /connecttest.txt");
  });
